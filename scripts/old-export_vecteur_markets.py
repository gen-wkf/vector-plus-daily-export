...
The script:
1. Reads token + clients/applications from auth JSON (default: responses/auth_clients.json)
...
DEFAULT_AUTH_FILE = Path("../responses/auth_clients.json")
DEFAULT_HTTP_FILE = Path("../app.http")
DEFAULT_OUTPUT_PREFIX = Path("../responses/all_markets")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--token", type=str, default=None, help="Override token from auth-file.")
...
def resolve_base_url(args: argparse.Namespace) -> str:
    ...
    pattern = re.compile(r"@base_url_vecteur_plus\s*=\s*(\S+)")
    try:
        content = args.http_file.read_text(encoding="utf-8")
...
def request_json(
    ...
    """Wait 5s before each request and then execute it."""
    time.sleep(5)
...
def build_search_payload(page: int, size: int, include_facets: bool) -> dict[str, Any]:
    ...
    if include_facets:
        payload["facets"] = DEFAULT_FILTERS
    return payload
...
def main() -> int:
    ...
    try:
        auth_data = read_json_file(args.auth_file)
    except OSError as exc:
        print(f"Unable to read auth file {args.auth_file}: {exc}", file=sys.stderr)
        return 1
    except json.JSONDecodeError as exc:
        print(f"Invalid JSON in auth file {args.auth_file}: {exc}", file=sys.stderr)
        return 1

    token = args.token or auth_data.get("token")
    if not token:
        print("No token found. Provide --token or update auth-file with a valid token.", file=sys.stderr)
        return 1

    clients = auth_data.get("clients")
    if not isinstance(clients, list):
        print("auth-file must contain a 'clients' array.", file=sys.stderr)
        return 1
